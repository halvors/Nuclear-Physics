...
public abstract class MachineInput<INPUT extends MachineInput<INPUT>> {
    public static boolean inputContains(ItemStack container, ItemStack contained) {
        if (container != null && container.stackSize >= contained.stackSize) {
            if (OreDictionaryHelper.getOreDictName(container).contains("treeSapling")) {
                return InventoryUtility.equalsWildcard(contained, container);
            }

            return InventoryUtility.equalsWildcardWithNBT(contained, container) && container.stackSize >= contained.stackSize;
        }
