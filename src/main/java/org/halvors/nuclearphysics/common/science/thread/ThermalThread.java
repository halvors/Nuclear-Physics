package org.halvors.nuclearphysics.common.science.thread;
...
public class ThermalThread extends Thread {
    ...
    public static double getTemperature(final World world, final BlockPos pos) {
        ...
        if (thermalSourceMap.containsKey(key)) {
            return thermalSourceMap.get(key);
        }
    ...
    */
    ...
    public void run() {
        while (canUpdate()) {
            for (Pair<World, BlockPos> key : thermalSourceMap.keySet()) {
                ...
                NuclearPhysics.getProxy().addScheduledTask(() -> {
                    ...
                    if (currentTemperature < 0) {
                        ...
                        final ThermalEvent.ThermalUpdateEvent event = new ThermalEvent.ThermalUpdateEvent(world, pos, currentTemperature, deltaFromEquilibrium, DELTA_TIME, isReactor);
